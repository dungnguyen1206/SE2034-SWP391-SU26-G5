...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
        ...
        @Query(value = "SELECT a FROM Appointment a " +
                        ...
                        "ORDER BY sl.startTime DESC, a.createdAt DESC", countQuery = "SELECT COUNT(a) FROM Appointment a "
                                        +
                                        "JOIN a.patient p " +
                                        "WHERE a.bookingDate = :today " +
                                        "AND (:search IS NULL OR :search = '' " +
        ...
        @Query(value = "SELECT a FROM Appointment a " +
                        ...
                        +
                        "OR p.phone LIKE CONCAT('%', :search, '%')) " +
                        "ORDER BY a.bookingDate DESC, sl.startTime DESC, a.createdAt DESC", countQuery = "SELECT COUNT(a) FROM Appointment a "
                                        +
                                        "JOIN a.patient p " +
                                        "WHERE a.bookingDate BETWEEN :fromDate AND :toDate " +
                                        "AND (:status IS NULL OR :status = '' OR a.status = :status) " +
                                        ...
                                        +
                                        "OR p.phone LIKE CONCAT('%', :search, '%'))")
        Page<Appointment> searchAppointmentListForReceptionist(@Param("search") String search,
                        @Param("status") String status, @Param("fromDate") LocalDate fromDate,
        ...
        @Query(value = "SELECT DISTINCT a FROM Appointment a " +
                        ...
                        +
                        "OR p.phone LIKE CONCAT('%', :search, '%')) " +
                        "ORDER BY a.createdAt DESC", countQuery = "SELECT COUNT(DISTINCT a) FROM Appointment a " +
                                        "JOIN a.patient p " +
                                        "WHERE a.bookingDate = CURRENT_DATE " +
                                        "AND a.status != 'CANCELLED' " +
                                        "AND (:search IS NULL OR :search = '' " +
        ...
        @Query(value = "SELECT CASE WHEN COUNT_BIG(a.id) > 0 THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END " +
                        ...
                        "         AND ts.start_time < CAST(:targetEndTime AS TIME)))", nativeQuery = true)
        ...
        Page<Appointment> findByDoctorIdAndBookingDateAndStatusIn(Long doctorId, LocalDate bookingDate,
        ...
        long countByDoctorIdAndBookingDate(@Param("doctorId") Long doctorId,
                        @Param("bookingDate") LocalDate bookingDate);
