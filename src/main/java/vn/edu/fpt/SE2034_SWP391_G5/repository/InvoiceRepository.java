...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
        ...
        vn.edu.fpt.SE2034_SWP391_G5.dto.response.ReceptionistDashboardResponse getTodayInvoiceDashboardCounts(
        ...
        BigDecimal getTotalAmount(@Param("paymentStatus") String paymentStatus, @Param("month") int month,
                        @Param("year") int year);
        @Query("SELECT i FROM Invoice i WHERE i.appointment.patient.id = :patientId AND i.paymentStatus = :paymentStatus")
        List<Invoice> findByPatientIdAndPaymentStatus(@Param("patientId") Long patientId,
        ...
        Optional<InvoiceSummaryResponse> getTotalAmountByPaymentStatus(@Param("paymentStatus") String paymentStatus,
                        ...
                        @Param("endDate") LocalDateTime endDate, @Param("month") Integer month,
        ...
        @Query("select new vn.edu.fpt.SE2034_SWP391_G5.dto.response.InvoiceRowResponse(" +
                        ...
                        " Where (:month is null or month(coalesce(i.paidAt, i.updatedAt, i.createdAt))=:month)" +
                        " and (:year is null or year (coalesce(i.paidAt, i.updatedAt, i.createdAt))=:year)" +
                        " and ((:startDate is null and :endDate is null) or coalesce(i.paidAt, i.updatedAt, i.createdAt) between :startDate and :endDate)")
